...
class Authority():
    ...
    def gen_cert(self, cert_file, signing_key):
        ...
        SAN_override = False
        BasicConstraints_override = False
        if hasattr(self, 'x509_extensions'):
            filtered = (x for x in self.x509_extensions if x.startswith("x509."))
            for x in filtered:
                if x.startswith('x509.SubjectAlternativeName'):
                    SAN_override = True
                    print(SAN_override)
                elif x.startswith('x509.BasicConstraints'):
                    BasicConstraints_override = True

                x = 'cert = cert.add_extension({})'.format(x)
                print(x)
                # compile(x, '', 'exec')
                exec(compile(x, '', 'exec'))

        if self._type in ('root', 'sign'):
            if not BasicConstraints_override:
